...
# Change composer.json so we are able to install Drual dev version
sed -i '33 c \    "minimum-stability": "dev",' composer.json
sed -i '39 c \         "composer-exit-on-patch-failure": true,\n         "patchLevel": {\n         "drupalcore": "-p2"\n        },' composer.json

# Not sure if this is needed
lando composer require cweagans/composer-patches:~1.0 --update-with-dependencies

# Require Drupal dev version 
lando composer require drupal/core-recommended:9.4.x-dev --update-with-all-dependencies
